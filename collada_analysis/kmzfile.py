...
import zipfile, os, time, collada_analysis, shutil, StringIO
...
def readZipModel(kmzfile, name):
    ...
    # 后缀
    postfix = ['dae', 'jpg', 'png', 'JPG', 'PNG', 'bmp', 'BMP', 'svg', 'SVG']
    mesh = None
    # 创建存放纹理文件夹的标识
    hasmkdir = 'no'
    texturespath = ''
    ...
    for boundgeom in mesh.scene.objects('geometry'):
        ...
        for boundprim in boundgeom.primitives():
            ...
            if isinstance(boundprim, collada_analysis.triangleset.BoundTriangleSet):
                # type: <class 'collada_analysis.triangleset.TriangleSet'>
                boundprimType = 'boundtriangleset'

            elif isinstance(boundprim, collada_analysis.lineset.BoundLineSet):
                # type: <class 'collada_analysis.lineset.LineSet'>
...
def get_trianglesOrlines(boundprim, boundprimType):
    if boundprimType == 'boundtriangleset':
        for triangle in boundprim.triangles():
            # type(triangle)  <class 'collada_analysis.triangleset.Triangle'>
            yield triangle
    elif boundprimType == 'boundlineset':
        for line in boundprim.lines():
            # type(line)  <class 'collada_analysis.lineset.Line'>
            yield line
