...
class Surface:
    def __init__(self, elem):
        ...
        for e in elem.iter('{http://www.opengis.net/gml}posList'):
            ...
            # print " point_to_point_ds:",ds  #dian yu dian zhi jian de ju li
            # print " one_polygon_min_ds:", min(ds)# one poslist min bian
        self.minds = min(ds)
        self.ps = points
        # print 'points:', points
            ...
            if 4>=c:
                for elem in elems.iter('{http://www.opengis.net/gml}surfaceMember'):
                    ...
                    for e in elem.iter('{http://www.opengis.net/gml}exterior'):
                        ...
                        #minds_all shi yi ge wall zui duan bian de list
...
def fast_iter(context):
    ...
    for event, element in context:
        ...
        if element.tag.endswith('Building'):
            ...
            if element.get('{http://www.opengis.net/gml}id') == 'GUID_3EYbphh5zBze18EkzdRCRZ':
    ...
    del context
    util.saveobj(total_ps, './citygml_data/obj/s4w_two_floors_inandoutwall.obj')
